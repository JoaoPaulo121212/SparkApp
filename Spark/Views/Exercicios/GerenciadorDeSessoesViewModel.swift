...
class GerenciadorSessoesViewModel: ObservableObject {
    ...
    init() {
        ...
        carregarHistoricoTreinosConcluidos() // Essencial carregar o histórico
    ...
    func registrarSessaoIndividualConcluida(idSessaoConcluida: UUID, dataConclusao: Date = Date()) { //
        ...
        if let ultimaSessaoTS = dataUltimaSessaoIndividualConcluidaTS, //
           calendario.isDateInToday(Date(timeIntervalSinceReferenceDate: ultimaSessaoTS)) { //
            ...
            // Se quiser permitir sobrescrever, remova o return e ajuste a lógica de add/remove no histórico abaixo.
            // return
        }
        ...
        // LÓGICA PARA HISTÓRICO DETALHADO (ESSENCIAL)
    ...
    func obterDadosSemanaAtualParaPerfil() -> [InfoDiaTreinoParaPerfil] { //
        ...
        let diaDaSemanaDeHoje = calendario.component(.weekday, from: hoje) // 1 para Dom, 2 para Seg, etc. em pt_BR
        
        print("DEBUG ViewModel: INÍCIO obterDadosSemanaAtualParaPerfil(). Histórico tem: \(historicoTreinosConcluidos.count) itens.") // << NOVO PRINT
        ...
        // Adicionando prints para depuração da data inicial da semana
        ...
        for i in 0..<7 { // Itera de Domingo (i=0) a Sábado (i=6)
    ...
    private func criarTreinosIniciaisSeNecessario() { //
        ...
        for (nomeSessaoModelo, nomesExerciciosNoModelo) in treinosParaObjetivo { //
            ...
            for nomeExercicioString in nomesExerciciosNoModelo { //
                ...
                // ASSUMINDO QUE 'dadosExerciciosLocais' ESTÁ DEFINIDO GLOBALMENTE OU EM OUTRO LUGAR
